...
public class AuthServiceIntegrationTests
{
    ...
    public async Task RegisterAndLogin_Integration()
    {
        ...
        var email = $"registertest+{Guid.NewGuid():N}@example.com";
        ...
        finally
        {
            ...
            {
                await CleanupUserAsync(context, createdUserId.Value);
            }
        }
    }
    ...
    public async Task RegisterOrResend_Semvdberge_Email_Integration()
    {
        ...
        var email = $"semvdberge+{Guid.NewGuid():N}@gmail.com";
        ...
        if (settings == null)
        {
            Assert.True(true, "SMTP settings not configured; skipped SMTP integration test.");
            return;
        }
        ...
        try
        {
            ...
            var persisted = await context.Users.FindAsync(createdUserId!.Value);
        ...
        finally
        {
            if (createdUser)
            {
                await CleanupUserAsync(context, email);
            }
        }
    }
    ...
    public async Task Register_Semvdberge_Email_Integration()
    {
        ...
        if (settings == null)
        {
            Assert.True(true, "Local appsettings SMTP settings not configured; skipped registration email integration test.");
            return;
        }
        ...
        var testEmail = existing == null ? email : $"semvdberge+{Guid.NewGuid():N}@gmail.com";
        var createdUser = false;

        try
        {
            var result = await service.Register(testEmail, username, "Password1!", "127.0.0.1");
            Assert.True(result.IsSuccess, result.Error?.Message ?? "Register failed");
            Assert.Equal(testEmail, result.Value.Email);
            createdUser = true;

            var persisted = await context.Users.FirstOrDefaultAsync(u => u.Email == testEmail);
            Assert.NotNull(persisted);

            var subject = $"Registration test email to {testEmail} - {Guid.NewGuid():N}";
            var body = "This is a backend integration registration test email generated by AuthService.Register.";
            await emailService.SendNotificationEmailAsync(testEmail, subject, body);
        }
        finally
        {
            if (createdUser)
            {
                await CleanupUserAsync(context, testEmail);
            }
        }
    }
    ...
    public async Task SendVerificationEmailAsync_Semvdberge_Email_Integration()
    {
        ...
        if (settings == null)
        {
            Assert.True(true, "SMTP settings not configured; skipped email integration test.");
            return;
        }
    ...
    public async Task RegisterWithRealSmtp_Semvdberge_Email_Integration()
    {
        ...
        if (settings == null)
        {
            Assert.True(true, "SMTP settings not configured; skipped real SMTP integration test.");
            return;
        }
        ...
        if (HasIntegrationConnection())
        {
            ...
            await RunRegisterAgainstContext(contextFromEnv, email, username, emailService);
            return;
        }
    ...
    private static async Task RunRegisterAgainstContext(AppDbContext context, string email, string username, IEmailService emailService)
    {
        ...
        await CleanupUserAsync(context, result.Value.Id);
    }

    private static async Task CleanupUserAsync(AppDbContext context, Guid userId)
    {
    ...
    private static async Task CleanupUserAsync(AppDbContext context, string email)
    {
